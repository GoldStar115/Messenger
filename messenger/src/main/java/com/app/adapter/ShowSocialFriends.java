...
public class ShowSocialFriends extends BaseAdapter {
    ...
    public View getView(final int arg0, View view, ViewGroup container) {
        try {
            ...
            final ImageButton fbLink = (ImageButton) view.findViewById(R.id.fbLink);

            name_.setText(friendInfoCurrent_.getName());
            fbLink.setTag(friendInfoCurrent_.getId());

            if (friendInfoCurrent_.getMobile_no() != null && !friendInfoCurrent_.getMobile_no().equals("")) {
                try {
            ...
            if (friendInfoCurrent_.getLastseen() != null && !friendInfoCurrent_.getLastseen().equals("")) {
                TextView txtV_lastSeen = (TextView) view.findViewById(R.id.txtV_lastSeen);
                txtV_lastSeen.setVisibility(View.VISIBLE);
                txtV_lastSeen.setText("Last seen : " + friendInfoCurrent_.getLastseen());
            }
            ...
            if (singleChat_) {
                ...
                if (!groupMates_) {
                    ...
                    fbLink.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            context_.startActivity(new GlobalUtills().getOpenFacebookIntent(context_, fbLink.getTag().toString()));
                        }
                    });
                    fbLink.setFocusable(false);
                }
            ...
            if (context_ instanceof GroupUsers) {
